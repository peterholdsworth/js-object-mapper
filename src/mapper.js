...
Mapper.prototype.move = function(to, from, params, customjs){
  var expandFrom, expandTo;
  if (params === undefined){params = {};}
  if (params instanceof Function){customjs = params; params = {};} // handle missing params
...
Mapper.prototype.assign = function(to, value, params, customjs){
  if (params === undefined){params = {};}
  if (params instanceof Function){customjs = params; params = {};} // handle missing params
  customjs = customjs || identityFn;
...
Mapper.prototype.log = function(from){
  this.ops.push({op:log, from:expand(from)});
  return this;
};
Mapper.prototype.submap = function(to, from, params, mapping){
  if (params instanceof Mapper){ mapping = params; params = {}; } // handle missing params
  this.ops.push({op:submap, from:expand(from), to:expand(to), params:params, transform:mapping});
  add(this.to, to, mapping);
  add(this.paramTo, to, params);
  return this;
};
Mapper.prototype.execute = function(input, context, output, target){
  try {
  ...
  if (process.env.MAPPER_TESTS_GEN === 'true') {
    ...
    //{output, path, ctxBase}
    generator(input, context, output, target);
  }
  return out;
};
Mapper.prototype.map = function(input, context, output, target, index){
  output = output || {};
  context = context || {};
  var i;
  var t;
  for ( i=0; i<this.ops.length; i++){
    t = this.ops[i];
    // console.log(input, output, t.from, t.to, t.params, t.transform);
    // output = t.op.call(this, input, output, t.from, t.to, t.params, t.transform, context);
    target.push(t.to);
